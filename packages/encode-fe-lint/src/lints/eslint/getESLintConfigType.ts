 ...
 * @param cwd
 * @param pkg
 * @returns code-standard-eslint-config/index
 * @returns code-standard-eslint-config/react
 * @returns code-standard-eslint-config/typescript/index
 * @returns code-standard-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'code-standard-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
